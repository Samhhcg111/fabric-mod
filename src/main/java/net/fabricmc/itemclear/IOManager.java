...
public class IOManager {
    public static void genConfig(){
        ...
        if(!Files.exists(Paths.get("./config/ItemClearConfig.json"))){
            String gson = new GsonBuilder().setPrettyPrinting().create().toJson(new ConfigInstance());
            File file = new File("./config/ItemClearConfig.json");
            write(file, gson);
        }
    }
    public static ConfigInstance readConfig(){
        ...
        try{
            configInstance = new Gson().fromJson(new FileReader("./config/ItemClearConfig.json"), ConfigInstance.class);
        }
        catch(IOException e){
            e.printStackTrace();
